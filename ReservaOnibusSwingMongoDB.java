...
public class ReservaOnibusSwingMongoDB extends JFrame {
	...
	private void inicializarMongo() {
		...
		mongoClient = MongoClients.create("mongodb:YOUR_MONGODB_LOCAL_CHANGE_IT");
	...
	private void carregarMapa() {
		...
		for(int i = 0; i < capacidade; i++) {
			...
			botao.setBackground(reservado? Color.RED : Color.GREEN);
	...
	private void abrirJanelaDetalhesReserva(Document reserva) {
		...
		JDialog janelaDetalhes = new JDialog(this,
				"Detalhe da Reserva(lugar" + lugar+")",true);
		...
		btnFinalizar.addActionListener(e -> {
			...
			"(O assento volta a ficar livre, mas reserva continua no historico)",
			...
			if(resp == JOptionPane.YES_OPTION) {
				...
				String novoStatus = txtStatus.getText().trim();
		...
		btnDeletar.addActionListener(e -> {
			int resp = JOptionPane.showConfirmDialog(janelaDetalhes, 
					"Deseja realmente deletar essa reserva do banco de dados?\n"+
			"Ela nao aparecera nem no mapa e nem no historico.",
			"Confirmar Delecao",
			JOptionPane.YES_NO_OPTION);
			...
			if(resp == JOptionPane.YES_OPTION) {
				...
				JOptionPane.showMessageDialog(janelaDetalhes, 
						"Reserva Deletada com sucesso!"+
								"Ela nao aparecera mais no mapa, nem na pesquisa.",
								"Sucesso",
								JOptionPane.INFORMATION_MESSAGE);
				janelaDetalhes.dispose();
				carregarMapa();
	...
	private void abrirJanelaReserva(int lugar,String data) {
		...
		JDialog janelaReserva = new JDialog(this, "Reserva Lugar"+ lugar, true);
	...
	private void abrirJanelaPesquisa() {
		...
		JLabel lblContagem = new JLabel("Total de registro : 0");
	...
	private void pesquisarEAtualizarTabela(
		...
		try {
			...
			if(!dataIniStr.isEmpty()) {
				dataIni = sdfLocal.parse(dataIniStr);
			...
			"Erro ao parsear datas, Verifique o formato");
			return;
		...
		if(!nomeStr.isEmpty()) {
			filtro.append("nome", new Document("$regex", ".*" +nomeStr+ ".*")
					.append("$options", "i"));
		}
		if(!cpfStr.isEmpty()) {
			filtro.append("cpf", new Document("$regex", ".*" +cpfStr+ ".*")
					.append("$options", "i"));
		}
		if(!lugarStr.isEmpty()) {
			try {
				...
				JOptionPane.showConfirmDialog(janelaPesquisa, 
						"Lugar deve ser um numero inteiro",
						"Erro",
